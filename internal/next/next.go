...
// hasRemainingAuto returns (hasAuto, parseable, corrupted) where hasAuto is true
// if the file has any ~ descriptions, parseable is true if a nav block was found,
// and corrupted is true if the block is malformed or unclosed.
func hasRemainingAuto(path string) (bool, bool, bool) {
	data, err := os.ReadFile(path)
	if err != nil {
		return false, false, false // missing file doesn't block
	}
	content := string(data)
	pr := navblock.ParseNavBlock(content)
	if !pr.Found {
		// Unclosed nav block (opener present but no closer) is also corrupted.
