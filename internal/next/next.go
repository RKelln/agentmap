...
func FlushState(taskListPath, repoRoot string) (FlushResult, error) {
	...
	for _, relPath := range paths {
		...
		hasAuto, parseable := hasRemainingAuto(absPath)
		if !parseable {
			fmt.Fprintf(os.Stderr, "warning: %s: nav block is unparseable — check manually\n", relPath)
			continue
		}
		if hasAuto {
...
// hasRemainingAuto returns (hasAuto, parseable) where hasAuto is true if the file
// has any ~ descriptions, and parseable is true if a nav block was found.
func hasRemainingAuto(path string) (bool, bool) {
	data, err := os.ReadFile(path)
	if err != nil {
		return false, false // missing file doesn't block
	}
