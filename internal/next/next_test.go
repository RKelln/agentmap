...
func TestFlushState_UpdateDoesNotRegrowTilde(t *testing.T) {
	dir := t.TempDir()
	taskListPath := makeTaskList(t, dir, []struct {
		relPath string
		checked bool
	...
	// File with a clean nav block: purpose is human-written, about is
	// intentionally empty (agent left it blank). update.File must NOT
	// re-introduce ~keywords into the empty about.
	content := `<!-- AGENT:NAV
purpose:token lifecycle authentication
nav[1]{s,n,name,about}:
15,8,##Token Exchange,
...
OAuth2 code-for-token flow with PKCE required.
`
	writeFile(t, filepath.Join(dir, authMD), content)
	...
	// Verify update.File did NOT regrow ~ on the empty about.
	data, _ := os.ReadFile(filepath.Join(dir, authMD))
	pr := navblock.ParseNavBlock(string(data))
	if !pr.Found {
		t.Fatal("nav block should still exist after update")
	}
	if navblock.IsAutoGenerated(pr.Block.Purpose) {
		t.Errorf("purpose should not have ~, got %q", pr.Block.Purpose)
	}
	for _, e := range pr.Block.Nav {
		if navblock.IsAutoGenerated(e.About) {
			t.Errorf("about for %q was regrown with ~: %q", e.Name, e.About)
		}
	}
}
