...
`
	...
	for _, entry := range block.Nav {
		if entry.Name == "##Section" && entry.About != "" {
			t.Errorf("empty description should remain empty after update, got %q", entry.About)
		}
	}
}

func TestFile_RegenAutoGeneratedAbout(t *testing.T) {
	dir := t.TempDir()

	oldContent := `<!-- AGENT:NAV
purpose:docs
nav[1]{s,n,name,about}:
14,12,##Section,~garbage
-->
# Doc
...
Content here with more words for keyword extraction. Token exchange flow using OAuth2 PKCE authorization code grant.
`
	path := writeTempFile(t, dir, "doc.md", oldContent)
	...
	if len(block.Nav) == 0 {
		t.Fatal("expected nav entry")
	}
	entry := block.Nav[0]
	// ~-prefixed garbage should be regenerated (either fresh ~keyword or empty
	// if there's nothing useful to extract). The key: not the original ~garbage.
	if entry.About == "~garbage" {
		t.Errorf("~-prefixed garbage should be regenerated, got %q", entry.About)
	}
}
