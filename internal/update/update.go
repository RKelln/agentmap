...
func Update(root, repoRoot string, cfg config.Config, dryRun, quiet, verbose bool) error {
	if repoRoot == "" {
		repoRoot = root
	}
	...
	for _, f := range files {
		...
		report, err := File(fullPath, cfg, dryRun, quiet, verbose, changedLines)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %s: %v\n", f, err)
			continue
		}
		if report != noChanges {
			// In non-verbose mode, suppress reports with no actionable entries
			// (e.g. "Updated: path (no changes)") — only show when verbose.
			isNoChangesReport := strings.HasSuffix(report, " (no changes)")
			if !verbose && isNoChangesReport {
				continue
			}
			anyChanged = true
			// Track whether any report contains auto-generated descriptions
			// so the warning can be printed once at the end.
			if !anyAutogen && strings.Contains(report, "(auto-generated; needs review)") {
				anyAutogen = true
			}
			if !quiet {
				fmt.Println(report)
...
func File(path string, cfg config.Config, dryRun, quiet, verbose bool, changedLines ...[]gitutil.LineRange) (string, error) {
...
func buildUpdatedBlock(oldBlock navblock.NavBlock, sections []parser.Section, _ []ReportEntry, lines []string, cfg config.Config, _, totalLines int) navblock.NavBlock {
	...
	for i, s := range sections {
		...
		if found {
			...
			// Regenerate ~-prefixed abouts on update (they're auto-generated
			// noise that may be stale or garbage). Truly empty abouts and
			// human-written (non-~) abouts are preserved as-is.
			if navblock.IsAutoGenerated(about) {
...
// collapsed into >hints or dropped by PruneNavEntries. The (Name, Start) key
// disambiguates duplicate heading names so that only entries that actually
// survived pruning are reported — avoiding false positives where one surviving
// instance of a duplicated name (e.g. multiple ###gpt-4o headings) unblocks
// all other instances of the same name that were pruned.
func filterHintedNewEntries(reports []ReportEntry, nav []navblock.NavEntry) []ReportEntry {
	surviving := make(map[string]bool, len(nav))
	for _, e := range nav {
		key := fmt.Sprintf("%s:%d", e.Name, e.Start)
		surviving[key] = true
	}
	...
	for _, r := range reports {
		if r.Type == ReportNew {
			key := fmt.Sprintf("%s:%d", r.Name, r.NewStart)
			if !surviving[key] {
				continue
			}
		}
