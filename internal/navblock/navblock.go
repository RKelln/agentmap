...
// frontmatter, or -1 if the file has no frontmatter. The dirty bool is true
// when the closing line has trailing content after the delimiter (e.g.
// "---<!-- AGENT:NAV").
func FindFrontmatterEnd(lines []string) (int, bool) {
...
// indicating auto-generated keyword descriptions. Leading whitespace is
// tolerated (e.g. " ~keyword" from accidental editor insertion).
func IsAutoGenerated(s string) bool {
	return strings.HasPrefix(strings.TrimSpace(s), AutoGeneratedPrefix)
}

// TrimAutoGenerated removes the ~ prefix marker if present.
// Leading whitespace is also trimmed before removal.
func TrimAutoGenerated(s string) string {
	trimmed := strings.TrimSpace(s)
	if strings.HasPrefix(trimmed, AutoGeneratedPrefix) {
		return strings.TrimPrefix(trimmed, AutoGeneratedPrefix)
	}
	return trimmed
}
