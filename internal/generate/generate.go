...
// When force is false, files that already have a nav block are skipped.
func Generate(root string, cfg config.Config, dryRun, force bool) error {
	...
	for _, f := range files {
		...
		report, err := File(fullPath, cfg, dryRun, force)
		if err != nil {
...
// When force is false (the default), files that already have a nav block are skipped
// and SkippedExisting is returned. Pass force=true to overwrite existing nav blocks.
// If outputPath is non-empty, the result is written there instead of modifying the source.
func File(path string, cfg config.Config, dryRun, force bool, outputPath ...string) (string, error) {
	...
	if blockStart >= 0 {
		...
		// Skip files that already have a nav block unless force is set.
		if !force {
	...
	if contentLines < cfg.MinLines || len(headings) == 0 {
		...
		blockText = navblock.RenderPurposeOnly(purpose, totalLines)
		...
		placeholder := navblock.NavBlock{Purpose: purpose, Lines: totalLines, Nav: cappedForOffset}
	...
	// total line count from what was estimated before insertion.
	if totalLines > 0 {
		finalTotalLines := strings.Count(newContent, "\n")
...
// BuildAbout generates an auto-prefixed keyword description for the given section
// content. Returns "" if no keywords are found. This is the same logic used by
// buildNavEntries and is exported so update can fill empty About fields.
func BuildAbout(sectionContent string) string {
...
// BuildAboutFromLines is a convenience wrapper that extracts section content
// from a pre-split lines slice and delegates to BuildAbout.
func BuildAboutFromLines(lines []string, start, end int) string {
	return BuildAbout(getSectionContent(lines, start, end))
}
...
func PruneNavEntries(entries []navblock.NavEntry, subThreshold, expandThreshold, maxEntries int) []navblock.NavEntry {
	...
	// Strip any previously-appended subsection hints from About fields so that
	// re-running PruneNavEntries (e.g. via update) does not duplicate them.
	// Hints are the ">..." suffix. We strip them from any auto-generated (~) About
	// (including ~keywords>hints) and from empty-/prefixed-only abouts (">hints").
	// Human-written abouts (no ~ prefix) are left untouched.
	for i, e := range work {
		if idx := strings.Index(e.About, ">"); idx >= 0 {
			before := e.About[:idx]
			if before == "" || navblock.IsAutoGenerated(before) {
				work[i].About = before
			}
		}
	}
		...
		for _, c := range all {
			...
			if c.hintable {
				...
				for j := idx - 1; j >= 0; j-- {
					if entryNameDepth(work[j].Name) < maxD {
						p := &work[j]
						// Never append hints to human/agent-written abouts (no ~ prefix).
						// These are editorial content and must not be auto-modified.
						if p.About != "" && !navblock.IsAutoGenerated(p.About) {
							break
						}
						switch {
						case p.About == "":
