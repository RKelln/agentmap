...
func PruneNavEntries(entries []navblock.NavEntry, subThreshold, expandThreshold, maxEntries int) []navblock.NavEntry {
	...
	// Hints are the ">..." suffix; they only appear in auto-generated (~) About
	// values or ones that were purely hint-generated (About == "~>...").
	for i, e := range work {
		if idx := strings.Index(e.About, ">"); idx >= 0 {
			before := e.About[:idx]
			// Strip only when nothing precedes the ">" except the auto-prefix.
			if before == "" || before == navblock.AutoGeneratedPrefix {
				work[i].About = ""
			}
		}
	}
