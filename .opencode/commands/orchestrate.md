...
Phase 7: LAND           -- Documentation, rebase, merge to main, close beads, push
...
| Build/test (`make test`, `make lint`) | Code review -> `@general` |
...
2. **Read the design doc** -- `agentmap-design.md` contains the full specification.
   Identify which sections are relevant to this work.
3. **Explore** -- Delegate to `@explore`: find related code, patterns, files to change.
   ...
   Remind to only explore, not plan or implement.
...
1. **Restate requirements** from beads, design doc sections, and exploration.
2. **Implementation steps** -- ordered by dependency. Each: file path, changes, why.
3. **Testing strategy** -- unit tests, table-driven, edge cases; in what order.
...
2. `make lint` -- verify linting passes
...
1. **CI gate:** `make test && make lint && make build` -- fix failures before continuing.
2. **Code review** -- Delegate to `@general`: review `git diff main...HEAD`,
   check quality/idioms/errors/tests/performance. Include Reference File paths.
...
4. **Re-run CI** after fixes: `make test && make lint && make build`
...
1. **Documentation** -- If behavior changed, update AGENTS.md, README, or other docs.
   Delegate to `@general` if substantial. Include the no-commit instruction.

2. **Present changes** -- Summarize all changes to the user. **STOP.** Wait for sign-off.

3. **Merge** -- Only after explicit user approval:
```bash
# Rebase
git checkout main && git pull && git checkout - && git rebase main
make test && make lint && make build    # re-verify after rebase
