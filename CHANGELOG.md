...
on the same file, check off the wrong entry, or silently regrow `~`-prefixed descriptions.

### Fixed
- Corrupted nav blocks now block FlushState instead of being silently skipped,
  preventing infinite loops where `next` re-emits the same corrupted file every call
- `CheckOffTaskEntry` no longer matches the wrong entry's checkbox — the `- [ ]`
  search is now bounded to each entry's `## ` heading range
- `update.File` no longer silently re-adds `~keyword` to `about` fields the agent
  intentionally left empty; `~`-prefixed entries still get regenerated on update
- `IsAutoGenerated` and `TrimAutoGenerated` now trim leading whitespace before
  checking the `~` prefix, preventing accidental bypass from editor insertion
